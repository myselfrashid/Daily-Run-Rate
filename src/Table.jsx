                    ...
                    {data.map((data, index) => {
                        ...
                        return (
                            <tr className='flex px-0 py-4 bg-slate-400 w-full items-center justify-between text-center' key={index}>
                                ...
                                <td className='lg:text-base md:text-xs xs:text-[7px] lg:w-32 md:w-20 xs:w-8 lg:font-normal xs:font-thin'><ul>{data.excludedDates.map((date, index) => (<p key={index}>{date}</p>))}</ul></td>
